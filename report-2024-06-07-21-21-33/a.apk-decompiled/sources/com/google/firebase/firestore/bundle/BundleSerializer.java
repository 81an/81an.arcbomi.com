...
public class BundleSerializer {
    ...
    private BundledQuery decodeBundledQuery(JSONObject jSONObject) throws JSONException {
        ...
        List<OrderBy> decodeOrderBy = decodeOrderBy(jSONObject2.optJSONArray(io.flutter.plugins.firebase.database.Constants.ORDER_BY));
        Bound decodeStartAtBound = decodeStartAtBound(jSONObject2.optJSONObject(io.flutter.plugins.firebase.database.Constants.START_AT));
        Bound decodeEndAtBound = decodeEndAtBound(jSONObject2.optJSONObject(io.flutter.plugins.firebase.database.Constants.END_AT));
        verifyNoOffset(jSONObject2);
        return new BundledQuery(new Target(decodeName, str, decodeWhere, decodeOrderBy, decodeLimit(jSONObject2), decodeStartAtBound, decodeEndAtBound), decodeLimitType(jSONObject));
    }

    private int decodeLimit(JSONObject jSONObject) {
        JSONObject optJSONObject = jSONObject.optJSONObject(io.flutter.plugins.firebase.database.Constants.LIMIT);
        if (optJSONObject != null) {
            return optJSONObject.optInt("value", -1);
        }
        return jSONObject.optInt(io.flutter.plugins.firebase.database.Constants.LIMIT, -1);
    }
